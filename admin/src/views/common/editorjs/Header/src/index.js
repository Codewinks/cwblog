...
class Header {
    ...
    normalizeData(data) {
        ...
        newData.alignment = data.alignment || this._settings.defaultAlignment || Header.DEFAULT_ALIGNMENT
    ...
    renderSettings() {
        ...
        wrapper.classList.add(this._CSS.settingsWrapper);

        this._settings.alignment.map(tune => {
            const button = document.createElement('div');
            button.classList.add('cdx-settings-button');
            button.innerHTML = tune.icon;
            button.classList.toggle(this._CSS.settingsButtonActive, tune.name === this.data.alignment);
            wrapper.appendChild(button);

            return button;
        }).forEach((element, index, elements) => {
            element.addEventListener('click', () => {
                this._toggleTune(this._settings.alignment[index].name);

                elements.forEach((el, i) => {
                    const {name} = this._settings.alignment[i];
        ...
        wrapper.appendChild(tagsHolder);
        ...
        if (this._settings.allowAnchor === true) {
             ...
             */
            wrapper.appendChild(settingsItemAnchor);
